...
std::vector<GLfloat> MyGL::computeGaussianKernel(int radius) {
    ...
    for(int y = -radius; y <= radius; ++y){
        for(int x = -radius; x <= radius; ++x){
            ...
            int remapped_idx = (y + radius) * kernelWidth + (x + radius);
            kernel[remapped_idx] = exp(exponent) / (2 * M_PI * sigma2);
            sum += kernel[remapped_idx];
        }
    }
...
void MyGL::renderSceneToGBuffer()
{
    ...
    std::vector<GBufferOutputType> drawBufferTypes{
    ...
    geometryBuffer.setDrawBuffers(drawBufferTypes);

    // 2. Set progMeshToGBuffer's model-view-projection matrix
    //    and camera position uniforms
    glm::mat4 matMVP = camera.getProj() * camera.getView() * glm::mat4(1.0f);
    progMeshToGBuffer.setUnifMat4("u_MVP", matMVP);
    ...
    // Set the Sampler2D in the shader to read from the texture slots
    ...
    // 5. (Already given to you) Bind the G-buffer and render to it
