...
std::string QLoadObj(std::vector<shape_t> &shapes,
                    std::vector<material_t> &materials, // [output]
                    const char *filename, const char *mtl_basepath) {
  ...
  return QLoadObj(shapes, materials, ifs, matFileReader);
}

std::string QLoadObj(std::vector<shape_t> &shapes,
                    std::vector<material_t> &materials, // [output]
                    QTextStream &inStream, MaterialReader &readMatFn) {
  std::stringstream err;
...
//  std::vector<char> buf(maxchars); // Alloc enough size.
  ...
  while (!inStream.atEnd()) { // CHANGED
    buf = inStream.readLine(); // CHANGED

    std::string linebuf(buf.toStdString()); // CHANGED
